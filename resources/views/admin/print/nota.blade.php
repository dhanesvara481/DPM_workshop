...
<div class="max-w-md mx-auto p-6">
    ...
    <div class="no-print flex items-center justify-between mb-4">
        <a href="{{ route('detail_riwayat_transaksi', $trx->id) }}"
    ...
    <div class="mt-4 text-sm space-y-1">
        ...
        <div class="flex justify-between">
            ...
            {{-- FIX: gunakan created_at yang mengandung jam --}}
    ...
    @php
        $subtotalHitung = 0;
        $jasaItems      = collect($items ?? [])->filter(fn($i) => ($i->tipe_transaksi ?? '') === 'Jasa');
        $barangItems    = collect($items ?? [])->filter(fn($i) => ($i->tipe_transaksi ?? '') !== 'Jasa' || !empty($i->barang_id));
    @endphp
    ...
    {{-- FIX: Ringkasan biaya dengan biaya jasa --}}
    @php
    ...
    <div class="text-sm space-y-1">
        ...
        {{-- FIX: Tampilkan biaya jasa jika ada --}}
        @if($biayaJasa > 0)
        <div class="flex justify-between">
