...
<head>
  ...
  <title>Detail Notifikasi - DPM Workshop</title>
    ...
    <section class="relative p-4 sm:p-6">
      <div class="max-w-[980px] mx-auto w-full">
        ...
        @php
          ...
          /** DUMMY DATA (untuk testing; hapus kalau backend sudah ada)
          $dummy = [
            1 => [
              'notifikasi_id' => 1,
          ...
          */
          ...
          // default kalau backend belum isi
          $notif = $notif ?? null;
        @endphp

        @if (!$notif)
          {{-- STATE: NOTIF TIDAK ADA --}}
          <div class="rounded-2xl border border-slate-200 bg-white/85 backdrop-blur
            ...
            <div class="px-6 py-6">
              ...
              <p class="mt-2 text-sm text-slate-600">
                Data notifikasi belum tersedia. (Kalau mau testing, aktifkan blok dummy data <span class="font-mono">/**/</span> di atas.)
              </p>
              <div class="mt-5">
                <a href="/tampilan_notifikasi"
        ...
        @else
          @php
            // icon mapping (sama seperti list)
            $type = strtolower($notif['jenis_notifikasi'] ?? '');
            ...
            $tglDibuat = isset($notif['tanggal_dibuat']) ? \Carbon\Carbon::parse($notif['tanggal_dibuat'])->format('d F Y') : '-';
            $tglKirim  = isset($notif['tanggal_dikirim']) ? \Carbon\Carbon::parse($notif['tanggal_dikirim'])->format('d F Y') : '-';
          @endphp
                  ...
                  <div class="min-w-0 flex-1">
                    <div class="flex flex-wrap items-center gap-x-3 gap-y-1">
                      <div class="text-xl sm:text-2xl font-extrabold text-slate-900">
                        {{ $notif['judul_notif'] ?? 'Judul Notif' }}
                      </div>
                      <div class="text-xl sm:text-2xl font-extrabold text-slate-900">|</div>
                      <div class="text-xl sm:text-2xl font-extrabold text-slate-900">
                        {{ $notif['jenis_notifikasi'] ?? 'Jenis Notif' }}
                      </div>
                    </div>
                    ...
                    <div class="mt-5 grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div class="rounded-xl border border-slate-200 bg-slate-50 p-4">
                        ...
                        <div class="mt-1 text-sm font-bold text-slate-900">{{ $tglDibuat }}</div>
                      </div>
                      <div class="rounded-xl border border-slate-200 bg-slate-50 p-4">
                        <div class="text-xs font-semibold text-slate-500">Tanggal Dikirim</div>
                        <div class="mt-1 text-sm font-bold text-slate-900">{{ $tglKirim }}</div>
                      </div>
                    </div>
                    ...
                    <div class="mt-5 flex flex-wrap gap-2">
                      ...
                      {{-- nanti kalau mau action lain (mark read / dll), taruh sini --}}
                      {{-- /** <button class="h-10 px-4 rounded-xl bg-slate-900 text-white text-sm font-semibold hover:bg-slate-800 transition">Tandai dibaca</button> **/ --}}
                    </div>
