...
<div class="min-h-screen flex">
 ...
 {{-- ================= SIDEBAR ================= --}}
  ...
  <main id="main"
          ...
          <form id="formBarang" method="POST"
                ...
                <div class="field md:col-span-2">
                    ...
                    <select id="satuan" name="satuan" required
                        ...
                        @php $sat = old('satuan', $barang->satuan ?? ''); @endphp
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <div class="relative">
                            ...
                            <input id="harga_beli" name="harga_beli" type="text" inputmode="numeric" required
                                value="{{ old('harga_beli', number_format((int)($barang->harga_beli ?? 0), 0, ',', '.')) }}"
                                placeholder="0"
                        ...
                        <div class="relative">
                            ...
                            <input id="harga_jual" name="harga_jual" type="text" inputmode="numeric" required
                                value="{{ old('harga_jual', number_format((int)($barang->harga_jual ?? 0), 0, ',', '.')) }}"
                                placeholder="0"
                                class="money w-full pl-10 pr-3 py-3 rounded-xl border border-slate-200 bg-white text-sm
                                        placeholder:text-slate-400
                                        focus:outline-none focus:ring-4 focus:ring-blue-900/10 focus:border-blue-900/30 transition">
    ...
    const backLink = document.querySelector('a[href*="mengelola_barang"]'); // tombol Kembali topbar
    const btnReset = document.getElementById('btnReset');

    // simpan snapshot awal (buat deteksi perubahan)
    ...
    let snap0 = form ? initialSnapshot() : "";

    const isDirty = () => form && snap0 !== initialSnapshot();

    // helper toast (pakai toast kamu)
    ...
    btnReset?.addEventListener('click', () => {
        ...
        form.reset();
        // update preview harga lagi
        if (typeof updatePreviewHarga === "function") updatePreviewHarga();

        // update snapshot awal (karena sudah balik)
        snap0 = initialSnapshot();
        showToast("Reset", "Perubahan dibatalkan.", "success");
    });

    // ===== Konfirmasi keluar (Kembali / pindah halaman) =====
    const confirmLeaveIfDirty = (e) => {
        if (!isDirty()) return;
        e.preventDefault();
        const ok = confirm("Perubahan belum disimpan. Yakin mau keluar?");
        if (ok) window.location.href = e.currentTarget.href;
    };
    backLink?.addEventListener('click', confirmLeaveIfDirty);
    ...
    form?.addEventListener('submit', (e) => {
        ...
        // setelah submit beneran, biarin Laravel proses
        // (kalau kamu masih preventDefault di tempat lain, hapus preventDefault itu)
    });

    // ===== Warning kalau close tab / refresh =====
    ...
    //   {{-- Mobile Sidebar --}}
        ...
        // sidebar active indicator MOBILE
