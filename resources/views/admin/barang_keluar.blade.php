...
@section('title', 'Barang Keluar - DPM Workshop')
...
<section class="relative p-4 sm:p-6">
  <div class="max-w-[1120px] mx-auto w-full space-y-5">
    ...
    <div class="rounded-2xl bg-white/85 backdrop-blur border border-slate-200
      ...
      <form id="formKeluar" action="{{ url('/barang_keluar/store') }}" method="POST" class="px-6 py-6">
        @csrf
...
<script>
  ...
  const syncBarangFields = () => {
    ...
    if (qtyInput) {
      ...
      if (stokNum > 0 && cur > stokNum) qtyInput.value = stokNum;
    }
  };
  ...
  // SUBMIT (modal + validasi qty <= stok)
  form?.addEventListener('submit', (e) => {
    ...
    if (qty > stokNum) {
      qtyInput?.classList.add('border-red-300', 'shake');
      setTimeout(() => qtyInput?.classList.remove('shake'), 300);
      showToast('Gagal', 'Jumlah keluar melebihi stok tersedia.', 'error');
      return;
    }
