...
_toggle_  <div class="absolute inset-0 bg-gradient-to-br from-slate-50 via-white to-slate-100"></div>
        ...
        <section class="relative p-4 sm:p-6">
            <div class="max-w-[1120px]">
                ...
                <div class="rounded-2xl bg-white/85 backdrop-blur border border-slate-200
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                            <div>
                                ...
                                <p class="mt-1 text-sm text-slate-500">
                                    Pilih barang dari data yang sudah diinput di “Kelola Barang” untuk di catat keluar.
                                </p>
                            </div>
                    ...
                    <form action="{{ url('/barang_keluar/store') }}" method="POST" class="px-6 py-6">
                        @csrf

                        {{-- Invoice tidak ditampilkan di UI, untuk kebutuhan backend kalau diperlukan --}}
                        <input type="hidden" name="opsi_invoice_backend" value="Invoice">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                            ...
                            <div class="md:col-span-5">
                                ...
                                <input type="number"
                                    ...
                                    class="w-full px-3 py-2.5 rounded-lg border border-slate-200 bg-white/90 text-sm
                                            focus:outline-none focus:ring-4 focus:ring-blue-900/10 focus:border-blue-900/30 transition">
                                <p class="mt-2 text-xs text-slate-500">Jumlah yang keluar tidak boleh lebih dari stok saat ini.</p>
                            </div>
                ...
                {{-- TABLE RIWAYAT BARANG KELUAR (opsional) --}}
                <div class="rounded-2xl bg-white/85 backdrop-blur border border-slate-200
        ...
        <script>
            ...
            // search clear button (riwayat)
            const inputKeluar = document.getElementById('searchKeluar');
            if (inputKeluar) {
                const wrap = inputKeluar.parentElement;
                ...
                const sync = () => btn.classList.toggle('show', inputKeluar.value.trim().length > 0);
                sync();

                inputKeluar.addEventListener('input', () => {
                    sync();
                ...
                btn.addEventListener('click', () => {
                    ...
                    inputKeluar.focus();
                    sync();
                    document.querySelectorAll('#tableKeluar tbody tr').forEach(tr => tr.classList.remove('hidden'));
                });
            }
            ...
            const qtyInput = document.querySelector('input[name="qty_keluar"]');
            ...
            const syncBarangFields = () => {
                ...
                if (stokTersedia) stokTersedia.value = String(stokNum);

                // max qty mengikuti stok (biar ga bisa input > stok)
                if (qtyInput) {
                qtyInput.max = stokNum > 0 ? String(stokNum) : "";
                }
            };
