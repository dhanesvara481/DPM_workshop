...
@php
  ...
  // NANTI ambil dari controller/DB
  // =========================

  $stockTotalItem = $stockTotalItem ?? 128;
  $stockLow       = $stockLow       ?? 4;
  $stockOut       = $stockOut       ?? 2;

  $txTodayAll     = $txTodayAll     ?? 1;
  $txTotalAll     = $txTotalAll     ?? 12;

  $MAX_EVENTS_PER_DAY = $MAX_EVENTS_PER_DAY ?? 4;

  // ✅ nanti controller kirim $events (sama dengan jadwal_kerja_staff)
  $events = $events ?? [
    now()->format('Y-m-d') => [
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3 text-sm">
        @for ($i = 0; $i < 7; $i++)
          ...
          <div class="rounded-xl border border-slate-200 bg-slate-50 p-3">
            ...
            <div class="text-xs rounded-lg px-2 py-1 {{ $info['class'] }}">
              {{ $info['label'] }}
            </div>
          </div>
        @endfor
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function dashHideModal(){
    ...
    // jangan remove overflow kalau popup masih kebuka
  ...
  document.addEventListener('keydown', (e) => {
    ...
    const detailOpen = dashDetailModal && !dashDetailModal.classList.contains('hidden');
...
{{-- ✅ MODALS (dipindah ke BODY lewat stack) --}}
@push('modals')
