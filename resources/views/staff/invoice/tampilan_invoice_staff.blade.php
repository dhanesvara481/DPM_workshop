...
<script>
  ...
  const cm = {
    ...
    open({ title='Konfirmasi', message='—', note='', okText='Ya', cancelText='Batal', tone='neutral' } = {}){
      ...
      if (note) {
        this.noteWrap.classList.remove('hidden');
        this.note.textContent = note;
      } else {
        this.noteWrap.classList.add('hidden');
      }
...
<script>
  // ================= INVOICE LOGIC + SELECT BARANG =================
  const fmtID = (n) => (isFinite(n) ? n : 0).toLocaleString('id-ID');
  const barangListRaw = Array.isArray(window.BARANGS) ? window.BARANGS : [];
  const barangList = barangListRaw.filter(b => Number(b?.stok ?? 0) > 0);
  ...
  // mark dirty untuk input di luar tabel juga
